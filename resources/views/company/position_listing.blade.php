        ...
        <div class="overflow-x-auto">
            <table id="corporate-position-table" class="corporate-position-table w-full">
                ...
                <tr class="mt-4 cursor-pointer"
                    ...
                    <td>
                        @if ($feature_user_score->user->isconnected($opportunity->id, $feature_user_score->user->id) !=
                        null && $feature_user_score->user->isconnected($opportunity->id,
                        $feature_user_score->user->id)->is_shortlisted == true)
                        <div
                        ...
                        @elseif ($feature_user_score->user->isconnected($opportunity->id, $feature_user_score->user->id)
                        != null && $feature_user_score->user->isconnected($opportunity->id,
                        $feature_user_score->user->id)->is_connected == 'connected')
                        <div
                        ...
                        @elseif ($feature_user_score->user->isviewed($opportunity->id, $feature_user_score->user->id) ==
                        null)
                        <div
                ...
                <tr class="mt-4 cursor-pointer staff-view"
                    ...
                    <td class="">Positron Biotechnology Innovation
                        (HK) Ltd.</td>
                    <td>
                        @if ($user_score->user->isconnected($opportunity->id, $user_score->user->id) != null &&
                        $user_score->user->isconnected($opportunity->id, $user_score->user->id)->is_shortlisted == true)
                        <div
                            class="cursor-pointer inline-block px-3 text-sm text-center  font-book text-gray-light3 border border-smoke-light rounded-2xl bg-gray-light1 ">
                            Shortlisted
                        </div>
                        @elseif ($user_score->user->isconnected($opportunity->id, $user_score->user->id) != null &&
                        $user_score->user->isconnected($opportunity->id, $user_score->user->id)->is_connected ==
                        'connected')
                        <div
                            class="cursor-pointer inline-block px-3 text-sm text-center font-book text-gray-light border border-lime-orange rounded-2xl bg-lime-orange ">
                            Connected
        ...
        <div class="relative mb-20">
            <div
                ...
                <div class="absolute opportunity-image-box cus_transform_50">
                    ...
                    <img src="{{ asset('uploads/profile_photos/' . $user_score->user->image) }}" alt="staff pic"
                        class="shopify-image">
                    @endif
                </div>
            </div>
