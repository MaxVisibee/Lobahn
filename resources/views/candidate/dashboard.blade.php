                ...
                <div class="flex lg:justify-center justify-start self-center lg:pl-0 pl-4">
                    <div class="">
                        ...
                        <p class="font-book text-lg text-gray-light1">Listed
                            ...
                            @if ($opportunity->isconnected($opportunity->job_id, Auth::id()) != null &&
                            ($opportunity->isconnected($opportunity->job_id, Auth::id())->is_shortlisted = true))
                            <span class="dot bg-gray-light1 inline-block align-middle rounded-full mx-3.5"></span>
                            Your profile was shortlisted last
                            {{ date('M d, Y', strtotime($opportunity->isconnected($opportunity->job_id,
                            Auth::id())->updated_at)) }}
                            @elseif ($opportunity->isconnected($opportunity->job_id, Auth::id()) != null &&
                            ($opportunity->isconnected($opportunity->job_id, Auth::id())->is_connected = true))
                            <span class="dot bg-gray-light1 inline-block align-middle rounded-full mx-3.5"></span>
                            You connected last
                            {{ date('M d, Y', strtotime($opportunity->isconnected($opportunity->job_id,
                            Auth::id())->updated_at)) }}
                            ...
                            {{ date('M d, Y', strtotime($opportunity->isEmployerviewed($opportunity->job_id,
                            Auth::id())->updated_at)) }}
                            @endif
                        </p>
        ...
        <div class="relative mb-20">
            ...
            <div
                ...
                <div class="m-opportunity-box__title-bar__height match-target ml-8 py-11 2xl:py-12">
                    @php
                    $matched_factors = $opportunity->matched_factors == null ? [] :
                    json_decode($opportunity->matched_factors); @endphp
            ...
            <div class="bg-gray-light rounded-sm rounded-t-none pt-8 sm:pt-0">
                <div class="match-company-box p-4 sm:p-12">
                    ...
                    <div class="text-sm sm:text-base xl:text-lg text-gray-light1 letter-spacing-custom">
                        <a href="{{ route('candidate.company', $opportunity->company->id) }}">{{
                            $opportunity->company->company_name }}</a>
                    ...
                    <div class="tag-bar sm:mt-7 text-xs sm:text-sm">
                        ...
                        <span
                            ...
                            $mykey->keyword()->keyword_name }}</span>
                        @endforeach
                    </div>
