...
@section('content')
    ...
    <div class="row m-b-10">
        <div class="col-lg-12">
            <div>
                <a class="btn btn-primary" href="{{ route('partners.create') }}"><i class="fa fa-plus"></i> Create
                    Partners</a>
            </div>
        </div>
    </div> <!-- end page-header -->
                        ...
                        <tbody>
                            @foreach ($data as $key => $partner)
                                <tr>
                                    ...
                                    <td>
                                        <img class=""
                                            src='{{ asset("uploads/partner_logo/$partner->partner_logo") }}'
                                            alt="{{ $partner->partner_name ?? '-' }}" width="35px" height="auto">
                                    </td>
                                    <td>
                                        <input class="form-control" style="width: 100px;" type="number"
                                            class="sorting" name="sorting_{{ $partner->id }}"
                                            id="sorting-{{ $partner->id }}" value="{{ $partner->sorting ?? '0' }}"
                                            data-id="{{ $partner->id }}">
                                    </td>
                                    <td>
                                        <a class="btn btn-success btn-icon btn-circle"
                                            href="{{ route('partners.show', $partner->id) }}"><i
