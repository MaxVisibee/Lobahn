...
@section('content')
    ...
    <div class="row m-b-10">
        <div class="col-lg-12">
            <div>
                <a class="btn btn-primary" href="{{ route('news_events.create') }}"><i class="fa fa-plus"></i> Create
                    Events</a>
            </div>
        </div>
    </div <!-- end page-header -->
                        ...
                        <tbody>
                            @foreach ($data as $key => $event)
                                <tr>
                                    ...
                                    <td>{{ date('h:m A', strtotime($event->event_time)) ?? '-' }}</td>
                                    <td>{{ $event->event_title ?? '-' }}</td>
                                    <td>{{ $event->creator->name ?? '-' }}</td>
                                    <td>
                                        <a class="btn btn-success btn-icon btn-circle"
                                            href="{{ route('news_events.show', $event->id) }}"><i
                                                class="fas fa-eye"></i></a>
                                        <a class="btn btn-warning btn-icon btn-circle"
                                            href="{{ route('news_events.edit', $event->id) }}"> <i
