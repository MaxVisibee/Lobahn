...
@section('content')
    <div class="w-full bg-gray-warm-pale corporate-member-premiumplan-container">
        <div class="relative">
            ...
            <div class="absolute  premium-content top-1/2 left-1/2">
                <p class="text-white md:text-5xl text-4xl md:whitespace-nowrap font-book text-center">
        ...
        <div class="lg:flex w-full mt-12">
            <div class="bg-gray lg:w-6/12 w-full relative flex justify-center py-20">
                ...
                <div class="flex mt-4  justify-center self-center">
                    ...
                    {!! $talentDiscovery->description_one ?? '' !!}
                </div>
            </div>
            <div class=" lg:w-6/12 w-full">
                <img class="w-full"
                    src="{{ asset('uploads/talent_discovery/' . $talentDiscovery->image_one ?? '') }}" />
            </div>
        </div>
        <div class="lg:flex w-full flex-row-reverse">
            <div
                ...
                {!! $talentDiscovery->description_two ?? '' !!}
            </div>
            <div class=" lg:w-6/12 w-full">
                <img class="w-full four-icon-img"
                    src="{{ asset('uploads/talent_discovery/' . $talentDiscovery->image_two ?? '') }}" />
            </div>
        </div>
        <div class="lg:flex w-full">
            <div class="bg-gray lg:w-6/12 h-auto w-full relative flex justify-center py-20">
                <div class="w-full text-center flex self-center justify-center corporate-premium-content-3">
                    ...
                    {!! $talentDiscovery->description_three ?? '' !!}
                </div>
            </div>
            <div class=" lg:w-6/12 w-full">
                <img class="w-full corporate-premium-content-3-img object-cover"
                    src="{{ asset('uploads/talent_discovery/' . $talentDiscovery->image_three ?? '') }}" />
            </div>
        </div>
        <div class="lg:flex w-full flex-row-reverse">
            <div class="bg-gray lg:w-6/12 h-auto w-full relative py-20 flex justify-center">
                ...
                {!! $talentDiscovery->description_four ?? '' !!}
            </div>
            <div class=" lg:w-6/12 w-full">
                <img class="w-full corporate-premium-content-4-img object-cover"
                    src="{{ asset('uploads/talent_discovery/' . $talentDiscovery->image_four ?? '') }}" />
            </div>
        </div>
    </div>
    <div class="lg:flex w-full flex-row ">
        <div class="bg-gray lg:w-6/12 premium-talent-desc-content-container w-full relative">
            ...
            {!! $talentDiscovery->description_five ?? '' !!}
        </div>
        <div class=" lg:w-6/12 w-full">
            <img class="w-full value-service-image1"
                src="{{ asset('uploads/talent_discovery/' . $talentDiscovery->image_five ?? '') }}" alt="image value sign"
                ...
                <div class="talent-monthly-card relative mt-8 md:mt-0 group">
                    ...
                    <div class="purchase-button-section mt-5">
                        ...
                        <button
                            @if (!Auth::user() && !Auth::guard('company')->user()) onclick="window.location='{{ route('login') }}'"
                        @else onclick="window.location='{{ route('talent-discovery-parchase') }}'" @endif
                            @if ($percentage_package->is_recommanded) class="bg-lime-orange purchase-btn hover:bg-smoke-dark hover:text-gray-pale text-base lg:text-lg
                        text-gray rounded-corner focus:outline-none w-full py-2 xl:py-4 letter-spacing-custom"
                        @else
                        class="bg-smoke-dark purchase-btn hover:bg-lime-orange hover:text-gray text-base lg:text-lg
